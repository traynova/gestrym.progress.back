...
func setupEnvironment(isLocalEnv bool) error {
	if isLocalEnv {
		logger.Info("[GESTRYM_PROGRESS] server started in development mode")
		...
		logger.Info("[GESTRYM_PROGRESS] server started in production mode")
...
func initServer() {
	address := viper.GetString("GESTRYM_PROGRESS_SERVER_ADDRESS")
	ginMode := viper.GetString("GIN_MODE")
	if address == "" {
		logger.Fatal("Server address env 'GESTRYM_PROGRESS_SERVER_ADDRESS' not set")
	}
	if ginMode == "" {
	...
	logger.Info("[GESTRYM_PROGRESS] Start server on -> %v", address)
