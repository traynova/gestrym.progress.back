...
type env struct {
	...
	GESTRYM_PROGRESS_SERVER_ADDRESS string `mapstructure:"GESTRYM_PROGRESS_SERVER_ADDRESS" validate:"required"`
...
func InitEnvironment(isLocalEnv bool) {
	envOnce.Do(func() {
		...
		if isLocalEnv {
			logger.Info("[GESTRYM_PROGRESS] servidor iniciado en modo local")
			viper.SetConfigFile(envLocalConfigFile)
		} else {
			logger.Info("[GESTRYM_PROGRESS] servidor iniciado en modo producción")
		}
